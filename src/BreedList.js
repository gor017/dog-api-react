...
function BreedList () {
  ...
  return (
    <div style = {{
      ...
      <TextField margin="dense" id="outlined-basic" label="Outlined" variant="outlined" onChange={(e) => {setSearchText(e.target.value)}} value={searchText}/>
      {isLoading ? <div className="loader"></div> : (
      <ul>

        <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
        {breedListArr.filter((breed) => breed.includes(searchText)).map((breed) => {
          ...
          return(
            <React.Fragment key={breed}>
              ...
              <ListItemButton style={{
                ...
                <ListItemText primary={breed} onClick={() => imgSrc(breed)}/>
                
                {!!subBreedArr.length && (isOpenData[breed] ? <ExpandLess onClick={() => {onItemClick(breed) }}/> : <ExpandMore onClick={() => {onItemClick(breed) }}/>)}
              </ListItemButton>
              ...
              >
                <List component="div" disablePadding>
                  {subBreedArr.map((subBreed) => {
                    return(<ListItemButton onClick={() => imgSrc(breed, subBreed)} key={subBreed} sx={{ pl: 4 }}>
